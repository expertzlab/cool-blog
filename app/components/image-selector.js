...
function ImageSelector(props) {
  ...
  const { addImage, removelmage, setThumbnail, imageList } = props;
...
const uploadToClient = (event) => {
	...
	const existinglmage = imageList?.find((img) => img.name === imageFile.name);
	if (existinglmage) {
...
<div className={styles["new-img-container"]}>
	...
	thumbnail" onClick={copyFileName}>
	S
	</span>
<input className={styles["inp-image-file"]} type="file" name="file"
accept="jpg, jpeg, .png" onChange={uploadToClient} ref={inputFileRef} />

{image && <img className={styles["new-img"]} src={objectURL} alt=""/>}
{!image && (
	<div className={styles["btn-select-image"]}
	onClick={selectimageHandler}>
		Select Image
	</div>
)}
<input type="text" readOnly className={styles["inp-copy"]}
